function show_NMoE_results_simulated(x, y, TMoE, klas, TrueStats, newSubFolder, x_out_low, x_out_up, y_out_low, y_out_up, show_single_fig)
...
if show_single_fig
    ...
    %h6 = plot(x_out_low,y_out_low,'o','b',[0.6 0.6 .6]);
    %h7 = plot(x_out_up,y_out_up,'o','b',[0.6 0.6 .6]);
    xlabel('x'), ylabel('y');
    ylim(yaxislim)
    hold off
    legend([h1, h4, h2(1), h5, h3(1)], 'data', ['True mean',' (NMoE)'],'True Experts', ...
        ['Estimated mean',' (NMoE)'],'Estimated Experts',...
        'Location','SouthWest');
    legend('boxoff')
    savefig(fullfile(newSubFolder, 'NMoE_superposition_estimations'))
...
elseif 6>nargin
    ...
    savefig(fullfile(newSubFolder, 'NMoE_gating_net_proba'))
    ...
    xlabel('x'), ylabel('y');
    ylim(yaxislim)
    savefig(fullfile(newSubFolder, 'NMoE_confidence_regions'))
    hold off
    ...
    savefig(fullfile(newSubFolder, 'NMoE_GT_means'))
    ylim(yaxislim)
    box on
    xlabel('x'), ylabel('y');
    ...
    box on
    xlabel('x'), ylabel('y');
    savefig(fullfile(newSubFolder, 'NMoE_estimated_means'))
    ...
    legend('boxoff')
    savefig(fullfile(newSubFolder, 'NMoE_loglikelihood'))
    box on;

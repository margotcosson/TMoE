function show_TMoE_results_simulated(x, y, TMoE, klas, TrueStats, newSubFolder, x_out_low, x_out_up, y_out_low, y_out_up, show_single_fig)
...
if show_single_fig
    ...
    %h6 = plot(x_out_low,y_out_low,'o','color',[0.1 0.8 .3]);
    %h7 = plot(x_out_up,y_out_up,'o','color',[0.1 0.8 .3]);
    xlabel('x'), ylabel('y');
    ylim(yaxislim)
    hold off
    legend([h1, h4, h2(1), h5, h3(1)], 'data', ['True mean',' (TMoE)'],'True Experts', ...
        ['Estimated mean',' (TMoE)'],'Estimated Experts',...
        'Location','SouthWest');
    legend('boxoff')
    savefig(fullfile(newSubFolder, 'TMoE_superposition_estimations'))
...
elseif 6>nargin
    ...
    savefig(fullfile(newSubFolder, 'TMoE_gating_net_proba'))
    ...
    xlabel('x'), ylabel('y');
    ylim(yaxislim)
    savefig(fullfile(newSubFolder, 'TMoE_confidence_regions'))
    hold off
    ...
    legend('boxoff')
    savefig(fullfile(newSubFolder, 'TMoE_GT_means'))
    hold off
    ...
    legend('boxoff')
    savefig(fullfile(newSubFolder, 'TMoE_estimated_means'))
    hold off
    ...
    legend('boxoff')
    savefig(fullfile(newSubFolder, 'TMoE_loglik'))
    box on;
    ...
    ylabel('Observed data log-likelihood');
    legend('TMoE log-likelihood');
    savefig(fullfile(newSubFolder, 'TMoE_loglikelihood'))
    legend('boxoff')
    box on;
end
end